...
var xspacing = 7;   // How far apart should each horizontal position be spaced
...
function renderWave() {
 ...
 if (freqValues[x] == 0) {
  var diameter = 3;
   ...
   var diameter = map(freqValues[x], 120, 256, 40, 80)*volume;
}
    else {
      var diameter = map(freqValues[x],120, 256, 20, 40)*volume;
    }
 

  fill(230,180, 255,50);
  ellipse(x*xspacing-50,((height/2)-50)+yvalues[x],diameter,diameter);

  fill(0,180, 255,50);
  ellipse(x*xspacing,((height/2)-100)+yvalues[x],diameter,diameter);

  fill(230,180, 255,50);
  ellipse(x*xspacing-50,((height/2)-150)+yvalues[x],diameter,diameter);
