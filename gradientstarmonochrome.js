...
function draw() { 
  ...
  for (i =0; i<stars.length; i++) {
    ...
    if (volume > .1) {
      stars[i].diameter = map(freqValues[i], 120, 512, 0, 10)*volume;
    } else {
      stars[i].diameter = map(freqValues[i], 120, 512, 0, 5)*volume;
    }
...
//  noFill();
  ...
  if (axis == Y_AXIS) {  // Top to bottom gradient
    for (var i = y; i <= y+h; i++) {
      ...
      var c = [lerp(c1[0],c2[0],inter),lerp(c1[1],c2[1],inter),lerp(c1[2],c2[2],inter), 255];   // lerpColor(c1, c2, inter);
  ...
  //}  
    ...
    for (var i = x; i <= x+w; i++) {
      ...
      var c = [lerp(c1[0],c2[0],inter),lerp(c1[1],c2[1],inter),lerp(c1[2],c2[2],inter), 255];
...
function Star() {
  ...
  this.radius = random(-width/1.2, width/1.2);
   ...
   // if (this.degree >= TWO_PI && this.degree <= PI) {
      this.x = centerX + (((this.radius)*2) * cos(radians(this.degree)));
      this.y = centerY + (((this.radius)*1.25) * sin(radians(this.degree + increment)));
