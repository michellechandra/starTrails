  ...
  for (i =0; i<stars.length; i++) {
    ...
    if (volume > .05) {
     ...
     // stars[i].degree = map(freqValues[i], 120, 256, -360, 360)*volume; 
      stars[i].increment = map(freqValues[i], 120, 256, 0, -180)*volume;
...
Star.prototype.update = function() {
 ...
 // this.x = centerX + (this.radius *  tan(cos(radians(this.degree + increment))));
 // this.y = centerY + (this.radius * tan(sin(radians(this.degree + increment))));
